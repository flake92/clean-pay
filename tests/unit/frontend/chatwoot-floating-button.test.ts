...
function LauncherCommitProbe({
  onCommit,
  scope,
}: {
  onCommit: (scope: string, label: string | null) => void;
  scope: string;
}) {
  useLayoutEffect(() => {
    const launcher = document.querySelector<HTMLElement>(
      ".clean-pay-chatwoot-launcher",
    );
    onCommit(scope, launcher?.getAttribute("aria-label") ?? null);
  }, [onCommit, scope]);
...
describe("SupportChatFloatingButton", () => {
  ...
  it("never renders the previous account as open after the identity changes", async () => {
    const toggle = installChatwoot();
    confirmCurrentIdentity();
    const committedLabel = vi.fn();
    const renderAccount = (config: ChatwootWidgetConfig, scope: string) => (
      createElement(
        SupportChatSessionBoundary,
        { authenticated: true, chatwootConfig: config },
        createElement(SupportChatFloatingButton),
        createElement(LauncherCommitProbe, { onCommit: committedLabel, scope }),
      )
    );
    const view = render(renderAccount(chatwootConfig, "current"));

    await screen.findByRole("button", { name: "Открыть чат поддержки" });
    act(() => window.dispatchEvent(new CustomEvent("chatwoot:opened")));
    await screen.findByRole("button", { name: "Закрыть чат поддержки" });

    view.rerender(renderAccount(otherChatwootConfig, "other"));

    expect(committedLabel).toHaveBeenCalledWith(
      "other",
      "Подключаем чат поддержки",
    );
    expect(committedLabel).not.toHaveBeenCalledWith(
      "other",
    ...
    expect(connectingButton.getAttribute("aria-expanded")).toBe("false");
