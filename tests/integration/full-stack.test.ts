...
async function registerWithEmail() {
  const jar: CookieJar = {};
  const email = `clean-pay-integration-${Date.now()}-${Math.random().toString(16).slice(2)}@example.com`;
...
const protectedEndpoints: Array<{ method: string; path: string; body?: unknown }> = [
  ...
  { method: "POST", path: "/api/bff/auth/email/request-verification", body: { email: "nobody@example.com" } },
  { method: "POST", path: "/api/bff/auth/email/confirm", body: { email: "nobody@example.com", code: "000000" } },
  { method: "POST", path: "/api/bff/auth/email/change", body: { email: "new@example.com" } },
  ...
  { method: "POST", path: "/api/bff/link/remnashop", body: { email: "nobody@example.com", password: "secret" } },
...
describe("real Docker integration stack", () => {
  ...
  it("serves authenticated account, support and passkey setup surfaces after Telegram login", async () => {
    ...
    expect(support).toMatchObject({
      enabled: true,
      email: "support@clean-pay.integration.localhost",
  ...
  it("covers subscription, device and payment business endpoints for an authenticated Telegram user", async () => {
    ...
    expect([200, 404, 409], JSON.stringify(await debugResponse(current))).toContain(current.status);
    ...
    expect([200, 400, 404, 409], JSON.stringify(await debugResponse(devices))).toContain(devices.status);
    ...
    expect([200, 400, 404, 409], JSON.stringify(await debugResponse(deleteDevices))).toContain(deleteDevices.status);
    ...
    expect([200, 400, 404, 409], JSON.stringify(await debugResponse(deleteDevice))).toContain(deleteDevice.status);

    // Проверяем: неверный промокод возвращает контролируемую доменную ошибку.
    const promocode = await postJson("/api/bff/subscription/promocode", { code: "CLEAN_PAY_INTEGRATION_MISSING" }, jar);

    await expectNot5xx(promocode, "POST /api/bff/subscription/promocode");
    expect([200, 400, 404, 409, 422], JSON.stringify(await debugResponse(promocode))).toContain(promocode.status);
    ...
    expect([200, 400, 404, 409], JSON.stringify(await debugResponse(reissue))).toContain(reissue.status);
    ...
    if (purchasable) {
      ...
      if (purchase.status === 200) {
        ...
        expect([400, 404, 409, 422], JSON.stringify(await debugResponse(purchase))).toContain(purchase.status);
      }
    }
    ...
    if (renewable) {
      ...
      expect([200, 400, 404, 409, 422], JSON.stringify(await debugResponse(extend))).toContain(extend.status);
    }
  });
