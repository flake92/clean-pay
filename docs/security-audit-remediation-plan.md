...
- A-001—A-004 из рабочего обсуждения признаны пользователем излишними, сняты с рассмотрения и не блокируют реализацию.
...
1. **UI-1 — представление устройств:** убрать из интерфейса HWID и необработанный User-Agent, показывать тип устройства, ОС и клиент с доступными версиями.
...
3. Преобразовывать начальный фрагмент `Client/version` в компактное значение, например `INCY 2.4.7` или `Happ 5.2.0`; служебный хвост User-Agent не показывать.
...
Условие завершения: интерфейс выдаёт компактные строки вида `iPhone 12 INCY 2.4.7` или `Windows Happ 5.2.0`, отдельно показывает ОС и не раскрывает технический HWID.

Статус: реализовано в `CHG-001`, автоматические проверки приведены в `TST-006`; ожидает ручной приёмки перед следующим пунктом.
