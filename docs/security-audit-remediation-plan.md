...
| 2026-08-03 | DEC-010 | Решение | Прямое указание пользователя; дефект тестового входа | DEC-004 частично заменено: восстановлен автоматический backend preflight после обязательного Turnstile; обычный вход существующего пользователя больше не требует e-mail-кода | Принято | Новый e-mail получает код только после создания аккаунта; recovery использует отдельный код; Passkey показывается только для выбранного локального пользователя | Пользователь |
| 2026-08-03 | CHG-025 | Исправление авторизации | Clean Pay source/tests + Remnashop `ab0b9e1` | Восстановлены password login/register endpoints, пошаговая форма, upstream account identify и account-bound Passkey | Завершено | WebAuthn options ограничены `allowCredentials`, challenge связан с `userId`, неверный пароль открывает recovery, единый Turnstile переиспользуется с новым token на каждом шаге | Codex |
| 2026-08-03 | FIND-035 | Дефект Telegram link | Журналы тестового стенда и source | Виджет привязки выпускал Turnstile token для `telegram_auth_start`, но общий Telegram start route проверял его как `auth_login`; корректный token отклонялся с `403` до перехода в Telegram | Исправлено | Для авторизованной привязки проверяется `telegram_auth_start`, для анонимного входа сохраняется `auth_login`; оба назначения покрыты route-тестом | Codex |
| 2026-08-03 | FIND-036 | Дефект авторизации | Production session audit, Clean Pay/Remnashop source | Полноценная Passkey-сессия подтверждённого e-mail пользователя не содержала upstream token bundle; отсутствие технических токенов ошибочно преобразовывалось в `EMAIL_REQUIRED`, поэтому пользователя отправляли на Telegram-only форму и просили пароль | Исправлено | FULL-сессия с подтверждённым e-mail автоматически получает upstream-сессию через закрытый service-to-service contract; e-mail и upstream owner проверяются до сохранения токенов; guided flow автоматически пропускает уже подтверждённый адрес | Codex |
| 2026-08-03 | DEC-011 | Решение авторизации | Требуемая продуктовая логика и повторный security review | Сопровождаемое добавление e-mail перед оплатой/продлением предназначено только для Telegram-пользователя без подтверждённого e-mail; уже подтверждённый пользователь не проходит повторный password step | Принято | Закрытый `/auth/service-session` требует отдельный auth service key, выдаёт сессию только существующему подтверждённому и незаблокированному пользователю; Clean Pay принимает результат только для FULL локальной сессии с совпадающими e-mail и upstream user ID | Пользователь |
| 2026-08-03 | CHG-026 | Исправление авторизации | Clean Pay + Remnashop source/tests | Удалён ложный запрос «пароля от e-mail», добавлено автоматическое восстановление upstream-сессии и readiness-проверка нового контракта | Завершено | Clean Pay: ESLint, typecheck, `703/703`; Remnashop: Ruff, mypy `542`, `182/182`; отдельные регрессионные тесты подтверждают Passkey recovery без password/link запроса и конкурентное схождение Telegram recovery | Codex |
| 2026-08-03 | FIND-037 | Дефект конкурентности | Production post-deploy logs | Параллельные BFF-запросы могли одновременно начать Telegram recovery: победивший запрос сохранял токены, ожидавший запрос видел изменённую сессию как ownership conflict и кратковременно возвращал ложный `409` | Исправлено | Ожидающий запрос повторно читает заблокированную сессию и принимает уже сохранённый валидный token bundle; настоящий identity conflict по-прежнему отклоняется | Codex |
