...
- devcontainer собирает Remnashop из полного immutable commit `da5dd1d9210b1ddb0cfdc7a7a01316bd9457e336` ветки направленного PR [`snoups/remnashop#135`](https://github.com/snoups/remnashop/pull/135), а локальный overlay стороннего исходного кода удалён. В PR добавлены только необходимые совместимые контракты: HTTP разрешён исключительно loopback cabinet URL для локальной разработки, а `BOT_API_BASE_URL` типизированно направляет aiogram в локальный Telegram mock; внешний HTTP, URL с credentials/query и production downgrade с HTTPS не допускаются;
...
- host E2E runner теперь bounded ждёт bootstrap до 120 секунд и сообщает понятный timeout вместо гонки с установкой зависимостей. Итоговые проверки: Prisma validate, typecheck, unit 383/383, integration 42/42, ESLint без ошибок (один известный warning generated coverage), production build 50/50, реальная миграция на непустой БД, immutable Remnashop Docker build и полный devcontainer E2E 104/104. Remnawave не изменялся; production rollout ещё не выполнялся.
...
- 2026-07-18: пункт 24 исправлен и проверен: devcontainer закреплён на полном SHA Remnashop `da5dd1d9210b1ddb0cfdc7a7a01316bd9457e336`, локальный source overlay удалён, а необходимые loopback/Bot API контракты прошли 13/13 тестов и Ruff и направлены только через PR `snoups/remnashop#135`. Три дублирующих индекса удалены bounded идемпотентной миграцией, отрепетированной на непустой PostgreSQL без потери данных; API docs автоматически совпадают со всеми route methods. Prisma validate, typecheck, unit 383/383, integration 42/42, ESLint без ошибок, production build 50/50 и реальный devcontainer E2E 104/104 прошли. Remnawave не изменялся; production rollout ещё не выполнялся.
