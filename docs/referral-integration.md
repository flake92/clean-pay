...
The reviewed Remnashop contract is PR #135 revision `428940c` with Alembic head `0054` (including the durable referral schema in `0052` and the bounded legacy recovery evidence in `0054`). API, worker, and scheduler must run that same image before referral traffic is enabled.
