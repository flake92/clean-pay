...
The reviewed Remnashop contract is PR #135 revision `9ff4092` with Alembic head `0053` (including the durable referral schema in `0052`). API, worker, and scheduler must run that same image before referral traffic is enabled.
