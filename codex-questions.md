...
## Перед шагом 12
- История платежей нужна.
- Проверки статуса на `success/fail/pending` нужны.
- Экран/блок поддержки нужен, но отключаемый.
- Поддержка настраивается через почту, Telegram username и FAQ URL.

## Перед шагом 13
- Нужен ли локальный audit log для payment actions: create, return-check, promocode, reissue, device-delete?
- Нужно ли добавить админский просмотр локальных платежей или только пользовательский кабинет?
- Нужны ли e-mail уведомления после успешной оплаты/продления?
