...
## Runtime режим
- Server: `npm run start:normal` на порту 4000.
- BFF всегда использует реальные зависимости из env.
...
- Limits are shared across the runtime BFF flows.
...
- Runtime rate limits use Redis; no in-memory fallback is used.


## ?????? ????? ?????? 16-23
- ?????? ????????? ? ???? ? ??????? ???????.
- ??? 16: audit ????? ? ?? `AuditLog` ??? ???????????????? ????????, ??????????? ?????? ? JSON ? `console`.
- ?????????? ??????/??????????? ???????? ??? `userId`, ? ?????????? e-mail ? metadata.
- `ipHash` ????????; ???? ????? ?? ?????????? `AUDIT_IP_HASH_SECRET` ? fallback ?? `WEB_JWT_SECRET`.
- Raw upstream error ? ?? ?? ?????: ?????? ??????????????? `code/status/path`, ??? ????????.
- `/devices` ????????? ????????? ?? ??????, ?????????? ???????????? ???????? ? `/cabinet`.
- ???????? CleanVPN ????????? ??????? ???????????.
- ?????? frontend warnings ?? stylesheet ? `<img>` ????? ??????.
- Production compose ????? ? ???? ?????????: ? Redis container ? ??? Redis container ??? ????????????? Redis.
- Production PostgreSQL ????????? ????? ???????????.
- Redis production: ??????? ??? ????? ????????, ??????? Redis ???????? ????? `REDIS_URL`.
- ?????? Nginx ?????????? Caddy; Caddy ??? ????????? SSL.
- Caddy ??????? 80/443.
- `/api/health` ????????? ???????????.
- Readiness ????????? Remnashop ?????? ???????? ? `/plans/public`.
- SMTP readiness ????????? ???????????; ???? ??????????, ????? ? ????.
- E2E/acceptance uses real configured services; real Remnashop remains covered by the manual checklist.
- ???????? ?????????? ????? ??????? ?? ?????????????.
- ???????????? ???????? ? `README.md`.
- README ????? ?? ??????? ? ??????????.
- Production deploy ????????? ??? ???? ?????? Docker Compose + Cloudflare Full Strict.


## ????? ????? 16-23
- ??? 16: ???????? ????? audit/logging helper, `AuditLog` ??? ???????????????? ???????, JSON console logs ??? ??????????? ??????, `ipHash` ????? `AUDIT_IP_HASH_SECRET`.
- ??? 17: ?????? frontend lint warnings ?? stylesheet ? `<img>`; theme ???????????? ????? CSS import, auth logo ????? `next/image`; `/devices` ????????? ????????? ?? ????????.
- Devcontainer: includes `mailpit`; app receives env for real configured dependencies.
- ???? 18-20: ????????? `Dockerfile`, `Caddyfile`, `docker-compose.prod.yml` ?? ?????????? Redis ? `docker-compose.prod.external-redis.yml` ??? ???????? Redis.
- ??? 21: `/api/health` ??????????? ?????????, `/api/health/readiness` ????????? DB, Redis, Remnashop `/plans/public`, SMTP TCP-connect.
- Step 22: added `scripts/smoke.sh` and smoke npm scripts.
- ??? 23: README ????????? ??? RU/EN operational guide ? env, deploy, Caddy/Cloudflare, update/rollback ? acceptance checklist.
- Checks: `npm run lint`; `npm run build`; production compose config; `smoke:normal`.
- ????????? ??????????????: Sass deprecation ?? Sakai `@import`; dev `next start` ????????????? ??? standalone output, ?? smoke ????????. Production Docker ????????? standalone server.
