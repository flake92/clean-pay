...
- SMTP readiness ????????? ???????????; ???? ??????????, ????? ? ????.
- E2E/acceptance ????????? ????? mock-mode, real Remnashop ???????? ????????? manual checklist.
- ???????? ?????????? ????? ??????? ?? ?????????????.
- ???????????? ???????? ? `README.md`.
- README ????? ?? ??????? ? ??????????.
...
- Devcontainer: ????????? ???????? dev-??????? `remnashop-mock`, `mailpit`, `turnstile-mock`; app ???????? env ?? ??? ?? ?????????.
...
- ??? 22: ???????? `scripts/smoke.sh` ? npm scripts `smoke:*`; normal/mock smoke passed.
- ??? 23: README ????????? ??? RU/EN operational guide ? env, deploy, Caddy/Cloudflare, update/rollback ? acceptance checklist.
- ????????: `npm run lint`; `npm run build`; `npm run build:mock`; production compose config for both variants; `smoke:normal`; `smoke:mock`.
