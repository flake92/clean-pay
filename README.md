...
Обновите Remnashop минимум до revision `0058`. Для существующей установки
мастер намеренно не навязывает revision `0059`: эта миграция меняет начальные
настройки e-mail-напоминаний и не должна молча перезаписывать уже сделанный
пользователем выбор.
...
Если после обновления вы отдельно решите перейти на Remnashop revision `0059`,
сначала изучите её аудит и backfill настроек уведомлений. Это отдельное решение:
обновление Clean Pay с `0.1.1` ограничивается безопасным минимумом `0058` и не
меняет пользовательские opt-out настройки.
