...
| --- | --- | --- | --- |
| [`#135`](https://github.com/snoups/remnashop/pull/135) | Открыт, target `dev` | Public/admin API, идемпотентные purchase/extend, durable recovery, сверку неоднозначных платежей, безопасное объединение пользователей и миграции `0046–0050`. | Единственный обязательный PR для полного платёжного recovery contract, `PAYMENT_RECONCILIATION_ENABLED=true` и полного сценария объединения e-mail/Telegram. Требует согласованного maintenance rollout всех HTTP/Taskiq/scheduler ролей. |
| [`#136`](https://github.com/snoups/remnashop/pull/136) | Закрыт автором | Независимый hardening password reset. | Отложен и намеренно не представлен как часть #135 или текущего production rollout. |
| [`#137`](https://github.com/snoups/remnashop/pull/137) | Закрыт как дубликат | Loopback-only HTTP для `WEB_CABINET_URL`. | Эквивалентное изменение уже находится в #135 (`b49e00f`). |
| [`#138`](https://github.com/snoups/remnashop/pull/138) | Закрыт как дубликат | Необязательный `BOT_API_BASE_URL`. | Эквивалентное изменение уже находится в #135 (`da5dd1d`). |
...
Remnashop `dev`. Дополнительный hardening из #136 закрыт и отложен как
отдельная задача. PR #137 и #138 закрыты как
дубли: соответствующие loopback URL и custom Telegram API endpoint уже входят
в #135. Проверенный commit #135 не включает hardening из #136.
...
проверенном commit `6aa25b5`. Это временный вариант для интеграционного
...
commit `6aa25b515b3e91a987ed65614dd8891d6a2355e3` как remote build context и
