...
Перед сборкой скрипт требует не менее 8 ГБ свободного места. Если места
недостаточно, он удаляет только неиспользуемый Docker build cache и висячие
образы, повторно проверяет диск и останавливается до сборки, если безопасный
резерв восстановить не удалось. Порог можно повысить переменной
`CLEAN_PAY_MIN_FREE_DISK_MB`; Docker volumes и данные баз не удаляются.
