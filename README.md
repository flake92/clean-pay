...
| `WEB_JWT_SECRET` | Yes | `change-me-long-random-web-jwt-secret` | Secret for web access/session tokens. |
| `WEB_REFRESH_SECRET` | Yes | `change-me-long-random-web-refresh-secret` | Secret for refresh/session tokens. |
| `AUDIT_IP_HASH_SECRET` | No | `change-me-long-random-audit-secret` | Secret for hashing IPs in audit logs. |
| `COOKIE_SECURE` | No | `true` | Use `true` for HTTPS. |
| `COOKIE_SAMESITE` | No | `lax` | SameSite policy: `lax`, `strict`, `none`. |
| `TELEGRAM_OIDC_ISSUER` | Yes | `https://oauth.telegram.org` | Telegram OAuth/OIDC issuer. |
| `TELEGRAM_OIDC_AUTHORIZATION_ENDPOINT` | Yes | `https://oauth.telegram.org/auth` | Telegram authorization endpoint. |
| `TELEGRAM_OIDC_TOKEN_ENDPOINT` | Yes | `https://oauth.telegram.org/token` | Telegram token endpoint. |
| `TELEGRAM_OIDC_JWKS_URI` | Yes | `https://oauth.telegram.org/.well-known/jwks.json` | Telegram JWKS endpoint. |
...
| `TURNSTILE_SITE_KEY` | No | empty | Build-time fallback for public Turnstile key. |
| `NEXT_PUBLIC_TURNSTILE_SITE_KEY` | When Turnstile is enabled | `1x00000000000000000000AA` | Public Turnstile key in frontend. Rebuild after changing it. |
