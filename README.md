...
Production запускается из зафиксированного commit, а не из меняющейся вершины
ветки. Пока тег `v0.2.0` не опубликован, текущий release-кандидат находится в
`update-nodejs`; после публикации замените значение на `refs/tags/v0.2.0`.

```bash
RELEASE_REF='refs/heads/update-nodejs'
...
Сохраните выведенный SHA в журнале выпуска. Тег предпочтительнее ветки: он
однозначно повторяет уже проверенный релиз.
...
Обновите Remnashop минимум до revision `0059`. Эта миграция включает
e-mail-напоминания всем существующим пользователям с подтверждённым адресом;
после обновления каждый пользователь по-прежнему может отключить их в профиле.
Неподтверждённые адреса не получают напоминания.
...
Revision `0059` выполняет backfill только для аккаунтов с подтверждённым e-mail:
включает им напоминания и сохраняет служебный аудит для контролируемого
восстановления. После миграции пользовательский переключатель продолжает
работать и позволяет отказаться от последующих писем.
...
После изменения выполните `./deploy.sh install`. Если хотя бы одно значение
отсутствует или контакт не прошёл безопасную проверку, кнопка чата не
показывается. Недоступность Chatwoot не останавливает кабинет и поэтому не
входит в основную readiness-проверку.
