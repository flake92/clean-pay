...
Сверка результатов платежей по умолчанию выключена. После установки совместимой версии Remnashop задайте `PAYMENT_RECONCILIATION_ENABLED=true` и сгенерируйте уникальный `PAYMENT_RECONCILIATION_SECRET` длиной не менее 32 символов. `deploy.sh` автоматически включит нужный Compose profile. Admin URL выводится из публичного Remnashop URL и не зависит от включения reconciliation; это необходимо также для безопасного объединения e-mail/Telegram аккаунтов. Явный `REMNASHOP_ADMIN_API_BASE_URL` нужен только как проверяемый override того же origin и API prefix.
...
Проверенный тестовый rollout от 20 июля 2026 года использует совместимый
Remnashop commit `b9da68a651e9ab0b7ed52d030e13754311614759`. Clean Pay,
PostgreSQL, Redis и retention worker проходят healthcheck; все dependency checks
internal readiness успешны. Платёжную reconciliation включайте только после
настройки admin API и отдельного fault-injection теста на disposable provider
stub. После maintenance rollout обязательно очистите
