...
2. Переключитесь на проверенный 40-символьный commit `0.2.0`, но не заменяйте
   существующий `.env` файлом `.env.example`. Определите точные публичные HTTPS
   origins, с которых Remnawave реально возвращает ссылки подписки, а также
   точные HTTPS origins всех реально используемых платёжных шлюзов. Выполните:
   ...
   Для нескольких origins передайте одну строку через запятую в соответствующем
   аргументе. Не копируйте пример платёжных шлюзов, если установка использует
   другие: проверьте фактические redirect URL в Remnashop. Команда отказывается
   угадывать оба списка, сохраняет `POSTGRES_USER`,
   `POSTGRES_PASSWORD` и все прикладные секреты, заменяет старый single-role
   `DATABASE_URL`, добавляет отдельные application/migration/retention/hold-роли
   и проверяет итоговый `.env`. До первой записи проверяются идентификаторы,
   пароль и точное соответствие legacy `DATABASE_URL`; каждая публикация
   `.env` выполняется атомарной compare-and-swap заменой. Весь процесс состоит
   из нескольких безопасно повторяемых публикаций: если питание пропало или
   оператор параллельно изменил файл, устраните причину и запустите ту же
   команду снова. Несовпадающий пароль, другая база, небезопасный или уже иначе
   заданный origin блокируются без изменений БД и контейнеров. Обычные `init`, `build`,
   `migrate` и `install` намеренно отклоняют неподготовленный `.env` `0.1.1`,
   чтобы не выполнить частичное неявное обновление.

   Уже запущенные контейнеры `0.1.1` продолжают использовать сохранённое при
   их создании окружение, но после подготовки не пересоздавайте их старым
   deployment-кодом. Если обновление отменено до изменения БД, восстановите
   authoritative `.env` `0.1.1` из зашифрованной копии; сгенерированные
   `.env.<role>` можно оставить до следующей подготовки или безопасно удалить
   только после точной проверки их путей.

   Разрешённые в `0.1.1` URL-параметры `statement_timeout` и
   `idle_in_transaction_session_timeout` переносятся в одноимённые
   role-scoped настройки `0.2.0`. Все legacy pool-параметры удаляются из URL:
   `application_name` теперь фиксирован по роли, а `connect_timeout`,
   `connection_limit` и `pool_timeout` фактически не применялись старым
   `PrismaPg` adapter. Неканонический timeout или конфликт с уже заданной новой
   переменной блокирует подготовку до изменения файла.
3. Сначала обновите Remnashop минимум до Alembic revision `0058`. Репозиторный
   порог нельзя занизить старым значением `0050`: preflight отклонит его до
   остановки Clean Pay. Затем подготовьте и проверьте образы командой
   `./deploy.sh build`. Она также проверяет наличие всех фактически используемых
   возможностей Docker Compose (`--wait-timeout`, `--pull`, `--policy`); при
   ошибке сначала обновите Compose v2 plugin.
4. Только после проверенного backup и непосредственно перед maintenance window
   ...
   запускает runtime. После успешных migration/grant `sync` оба одноразовых
   флага атомарно возвращаются в `false` до старта приложения во всех штатных
...
Подготовка legacy-конфигурации намеренно устанавливает
`PAYMENT_DATA_RETENTION_ENABLED=false`: новые миграции не должны сразу очищать
старые payment URL/provider snapshots, возраст которых был записан до появления
этой политики. Остальная автоматическая очистка продолжает работать. После
проверки сроков хранения, контрольной выборки старых платежей и резервной копии
включите платёжную политику явно и пересоздайте role-scoped runtime:
...
У новых установок `.env.example` содержит явное значение `true`. Отсутствующее
значение блокирует production validation, а напрямую запущенный legacy-worker
fail-safe не очищает платёжные данные.
