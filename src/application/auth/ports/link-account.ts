...
export interface LinkAccountReader {
  ...
  loadTelegramMergeConfirmation(userId: string): Promise<(TelegramMergeViewModel & { status: "PENDING" | "PROCESSING" | "COMPLETED" | "FAILED"; expiresAt: Date }) | null>;
}
...
export interface LinkAccountCommands {
  ...
  loadProviderProfile(session: { context: unknown }): Promise<{ email: string | null; emailVerified: boolean }>;
