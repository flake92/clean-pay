...
`POST /api/bff/link/remnashop`; JSON UTF-8 до 65 536 байт; доверенный origin; cookie session.
...
`400`, `401 AUTH_FAILED/UNAUTHORIZED`, `403`, `409 EMAIL_LINK_REQUIRES_VERIFICATION`, `ACCOUNT_MERGE_REQUIRED`, subscription/payment conflicts, `413`, `415`, `429`, `502`, `500`.
...
Ветка подтверждённого адреса:

```json
{"data":{"user":{...},"linked":true,"pendingVerification":false,"alreadyVerified":true}}
```

Ветка ожидающего подтверждения:

```json
{"data":{"user":{...},"emailVerification":{"success":true,"target_email":"...","expires_at":"..."},"linked":false,"pendingVerification":true}}
```

В pending user принудительно получает `is_email_verified:false`, `emailVerified:false`; Telegram IDs предпочитают локальные значения.
