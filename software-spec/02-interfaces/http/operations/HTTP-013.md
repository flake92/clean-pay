...
`POST /api/bff/auth/passkey/login/verify`; public JSON до 131 072 байт; доверенный origin; входные cookie не требуются и не определяют пользователя.
...
`200 {"data":{"success":true}}` плюс новая пара `clean_pay_access`/`clean_pay_refresh` с атрибутами из HTTP-002.
