...
JSON-объект, нормативно ожидается `code:string`. Реализация передаёт весь объект внешней системе: локально не проверяет тип/пустоту `code` и не удаляет дополнительные поля.

## Текущий транспорт

`POST /api/bff/subscription/promocode`; JSON UTF-8 до 65 536 байт; trusted origin; session cookie.

## Правила валидации

JSON обязан быть объектом; остальные правила задаёт RS-018. Пустое тело, malformed JSON и массив дают `400`; превышение размера — `413`.

## Нормализация

Отсутствует: объект передаётся как получен.
...
Attempted audit, RS-018, succeeded audit, возврат `{success,reward_type}`.
...
`200 {"data":{"success":boolean,"reward_type":string}}`.
