...
Цель каталога — сохранить продуктовые входы, доменные инварианты и внешние
интеграции Clean Pay без обращения к удалённому проекту. Текущая входная
архитектура зафиксирована ADR-003: server-rendered resourceful Rails.
...
- [`02-interfaces/http-api.md`](02-interfaces/http-api.md) — реестр 42
  канонических Rails-операций.
...
Факты спецификации были получены из route handlers, runtime validation, state
transitions и тестов контрольного среза до его удаления. ADR-003 намеренно
заменяет исторический BFF transport. Файлы `source-tree-manifest.md` и
`source-inventory.md` остаются доказательством удалённого приложения и не
описывают целевое Rails-дерево.
